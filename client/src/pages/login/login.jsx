    ...
    const onSubmit = (data) => {
        request.post("/login", data).then((res) => {
            if (res.data) {
                ...
                Cookies.set("cookiesToken", res.data)
                navigate("/");
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <div className={`flex flex-col mb-4 `}>
                <label className='text-[12px] mb-4 text-gray ' htmlFor="phone">Email</label>
                <input
                    type="text"
            ...
            <div className={`flex flex-col mb-[63px] ${errors.password ? 'has-error' : ''}`}>
                <label className='text-[12px] mb-4 text-gray ' htmlFor="password">Password</label>
            ...
            <Button type='submit' variant='primary' className='w-full py-[14px] text-[16px]'>Login</Button>
        </form>
    );
};
